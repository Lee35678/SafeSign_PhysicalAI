...
> `시스템_구성도_초안.md`를 종합해, **개발 착수를 위해 한 곳에서 볼 수 있도록
...
> 실물 테스트로 남겨둠(§11.2).~~
>
> **2026-09-22 갱신**: **RPi4B(8GB)·Pi Camera Module 3 입수로 전 부품 보유.** 위 "picar 전원 계통
> 분리 여부"도 **분리 확정**으로 해소됐다(11_하드웨어설계서_v1 §7.1). 미도착을 전제로 준비했던
> 1차 구현 임시 구성(RPi5=picar, PC=스테이션)은 **폐지**했다.
>
> **2026-09-20 갱신 6 (학습 데이터 확보)**: 공개 데이터셋 3종으로 **수신호 7종 27,735건 확보**
> (`services/data/build_dataset.py`). 이에 따라 **손모양 2건 변경**(우회전 유도 약지→소지,
> 확인/완료 따봉→주먹, 팀 승인), §3.2 수신호 표·§6 데이터 요구사항 갱신. **미해결 3건**(negative 0건,
> 치명 오분류 1건, 클래스 불균형 18배)을 §11.2에 등록. 라이선스 copyleft 주의사항 추가.
>
...
*(근거: 시스템_구성도_초안.md, 02_설계문서_v2 §4·§4-1)*
...
| 4 | 우회전 유도 | **엄지+소지 펴기** (2026-09-20 변경) | 우측 황색 LED 점멸 + 우회전 주행 | 크레인표준 · 🟡 |
| 5 | 확인/완료 | **다섯 손가락 접기(주먹)** (2026-09-20 변경) | 적색·황색 LED 번갈아 2회 점멸 후 소등 | 크레인신호수표준 · 🔴 |
...
>
> **2026-09-20 손모양 변경 2건** — 우회전 유도는 약지만 펴기가 사람이 따라 하기 어려워 소지로,
> 확인/완료는 공개 데이터셋에 "따봉" 대응 클래스가 없어 주먹으로 변경(팀 승인). 서보 각도표는
> 03_인터페이스계약서_v2 §5-1에 반영 — **`services/actuation`의 AI Hand 각도 상수도 같이 고쳐야 함.**
...
*(근거: 시스템_구성도_초안.md, 01_프로젝트계획서_v4 §역할 및 책임(RACI), 02_설계문서_v2 §1-1)*
...
- **클래스**: 8개 (수신호 7종 + negative) — 8개 클래스 모두 손 모양이 서로 다름
- **확보 현황 (2026-09-20)**: 공개 데이터셋 3종에서 **수신호 7종 27,735건 확보 완료**
  (목표 클래스당 300장 대비 전 클래스 충족). 전량이 vision의 `normalize.py`를 통과함.
  🔴 **negative는 0건** — 공개 데이터의 "손 없는 배경"은 랜드마크가 안 나와 쓸 수 없었음
- **분할**: **subject-wise split 강제**. 단 공개 데이터는 촬영자 정보가 없어(`subject_id=public` 단일)
  적용 불가 → **공개 데이터=학습용, 자체 촬영(외부인 1~2명)=KPI 평가용**으로 역할 분리
- **미해결 3건**: negative 확보 방침 · 치명 오분류 1건(KPI 0건 미달) · 클래스 불균형 18배 (§11.2)
- **라이선스 주의**: 사용 데이터 3종 중 2종이 **copyleft(ODbL·GPL 2.0)** — 원본 이미지 저장소 업로드
  금지, 출처 명시 필수, 모델 가중치 공개 범위는 팀 논의 필요
...
| 이슈 | 비고 |
| --- | --- |
| 🔴 **negative 클래스 확보 방침** (2026-09-20 신규) | 공개 데이터로는 0건. ① 7클래스 운영 + τ로 미판정 처리 ② "손은 있으나 7종 아닌 자세" 자체 촬영 — **팀 결정 필요** (04_데이터셋명세서_v2 §2-3) |
| 🔴 **치명 오분류 1건** (2026-09-20 신규) | 공개 데이터 검증에서 정지 308장 중 1건 오판정. KPI 목표 0건 미달 → "정지 전용 τ 상향"(05_모델카드_v3 §8-1) 적용 검토. 최종 판단은 자체 촬영 데이터 실측 후 |
| **클래스 불균형 18배** (2026-09-20 신규) | 정지 308장 vs 좌회전 5,629장. `class_weight="balanced"` 또는 per-class 상한으로 대응, 효과는 실측으로 확인 |
| picar 전원 계통 — 모터 노이즈가 Wi-Fi 모듈에 영향 주는지 | **2026-09-22 결정: 차체 배터리 단일 계통**(3S 11.1V → 강압 → 헤더 5V로 RPi4B 급전). 같은 날 오전의 "계통 분리"를 번복한 것이며, **레일을 공유하므로 노이즈 경로가 존재한다 → 벤치 테스트에서 실측**(모터 구동 중 Wi-Fi 왕복 + `vcgencmd get_throttled`). 용량은 제약이 아니다(모터 듀티 10%에서 약 3.8시간). 배선 규칙 반전: 헤더 5V는 급전 경로이며 **USB-C 동시 연결 금지** — 11_하드웨어설계서_v1 §7.1 |
| ~~RPi5 ↔ RPi4B Wi-Fi IP 구성(고정 IP/mDNS)~~ | **해소(2026-09-21)**: 공유기 없이 **RPi5를 AP로 띄우는 1:1 무선 직결 + 고정 IP**(RPi5 `192.168.50.1` / picar `192.168.50.10`). 발표장 공용 Wi-Fi 같은 통제 밖 변수를 시연 경로에서 제거 — 11_하드웨어설계서_v1 §6.1 |
...
| 06_테스트·평가리포트 | W4 실측 로그(1부)·집계 결과(2부) (현재 템플릿) |
| 08_리스크레지스터 | 상시 갱신 리스크 목록 |
| 09_화면목록 | 화면 단위 UX 흐름 |
| **11_하드웨어설계서** | **부품 사양·핀 배정·통신 규격·전원 제약을 통합한 하드웨어 단일 기준 문서 (초안). 부록 A~C에 전체 핀맵·부품 원본 스펙** |
| 12_AI비전_진행보고서 | vision 파트 진행 경과와 KPI 1차 실측 |
| 시스템_구성도_초안 | 논리 구성도 다이어그램(v5) |
| Git_설계서 | 브랜치 전략·저장소 운영 규칙 |
| 개발로그 | 일자별 작업·결정 이력 |
| proposals/ | 팀 결정 대기 중인 변경안 (picar 주행시간 스키마, web↔picar 통신 신뢰성) |
