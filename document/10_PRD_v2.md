...
| ~~picar 전원 계통 분리 여부(모터 노이즈가 Wi-Fi 모듈에 영향 주는지)~~ | **해소(2026-09-22)**: **분리 확정** — 모터는 Raspbot 배터리, Pi는 별도 어댑터. 레일을 공유하지 않으므로 노이즈 경로 자체가 사라진다(측정으로 판단할 문제가 아니라 구조로 제거). 대신 조립 전 **확장보드 헤더 5V 백피드 차단 확인**이 새 선결 조건 — 11_하드웨어설계서_v1 §7.1 |
...
| 09_화면목록 | 화면 단위 UX 흐름 |
| **11_하드웨어설계서** | **부품 사양·핀 배정·통신 규격·전원 제약을 통합한 하드웨어 단일 기준 문서 (초안)** |
