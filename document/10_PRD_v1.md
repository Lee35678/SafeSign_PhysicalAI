...
| SC-05 | 학습 완료/집계 | 필수 |
| SC-06 | 수료증 발급 | 필수 |
| SC-07 | 중간 이탈 재접속 | 필수(예외), 이어하기 여부 ⚠️ 미결 |
...
- 입력: RGB 프레임 (해상도/FPS ⚠️ 확정 필요 — 640×480부터 시작해 P95 판정지연 만족 최소치로 확정)
...
- **판정 안정화**: 프레임 단위 판정이 아닌 **N프레임 연속 동일 클래스**일 때 확정 (N값 ⚠️ 실측 후 확정).
  `LIVE_STREAM` 콜백에서 스레드-세이프 큐(deque)로 최근 클래스를 누적해 판단
- **판정 임계값 τ**: confidence < τ → `is_reject=true`. 결정 절차 — 검증셋 확률 수집 → τ 그리드 탐색 →
  "정지" 치명오분류 0건 최우선 필터 → 나머지 KPI 동시 만족 → 미판정률 최소값 채택 (05_모델카드_v3 §8)
...
| 채널 | 트리거 | 동작 | 근거 |
| --- | --- | --- | --- |
| AI Hand | `command: demo \| correct_pose` | 손가락 서보 5개 + 손목 서보 1개로 정답 수신호 시범/교정 자세 제시 | 02_설계문서_v2 §3, 03_인터페이스계약서_v2 §5-1 |
...
| micro:bit | 판정 결과(OK/NG + match_score) | 5×5 LED 매트릭스 O/X 표시, A/B 버튼으로 웹 화면 이동 | 02_설계문서_v2 §5, 03_인터페이스계약서_v2 §5-3 |
...
### 11.2 확정 필요 (⚠️ 개발 착수 전 우선 논의 권장)
| 이슈 | 관련 문서 |
| --- | --- |
| 파이프라인 방식 "결정" 칸 공식 기록 (랜드마크+SVM으로 사실상 확정됐으나 팀 회의록 미기재) | 02_설계문서_v2 §2 |
| 카메라 해상도/FPS 확정 (Camera Module 3 지원 모드 중 선택) | 03_인터페이스계약서_v2 §2 |
| micro:bit 시리얼 프로토콜 상세(구분자/종료문자/재전송) 확정 | 03_인터페이스계약서_v2 §5-3 |
| τ, N프레임, 서보 각도 실측값 확정 | 05_모델카드_v3 §8, 03_인터페이스계약서_v2 §5-1 |
| SC-07 세션 이어하기 구현 여부 | 09_화면목록_v2 |
| Macro F1 계산 범위(negative 포함 여부) 팀 재확인 | 05_모델카드_v3 §7-1 |
| picar 전원 계통 분리 여부(모터 노이즈가 RPi5/카메라에 영향 주는지) | 시스템_구성도_초안.md §5 |
