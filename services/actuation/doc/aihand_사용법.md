...
| `correct` | LED에 **O** 2초 표시 | `OK:CORRECT` |
| `incorrect` | LED에 **X** 2초 표시 | `OK:INCORRECT` |
...
1. **손가락 서보를 동시에 움직이지 않는다.** 펌웨어는 손가락 간 200ms 텀을 두고 순차 이동합니다.
...
- `correct`/`incorrect` 처리 중 2초간 `basic.pause()`로 블로킹되므로, 그 사이 들어온 BLE 명령은
  처리되지 않습니다.
