...
namespace StartbitV2 {
    ...
    */
    ...
    export function startbit_Init() {
        // startbit_initRGBLight();  // RGB 조명 기능 제거 (shim 컴파일 문제로 인해 비활성화, AiHand 제어에 불필요)
        serial.redirect(
            SerialPin.P12,
            SerialPin.P8,
        ...
        basic.forever(() => {
            ...
            if (0 < currentVoltage && currentVoltage < 6800 ) {
                basic.showIcon(IconNames.SmallDiamond); // 원래는 music.playTone() - BLE SoftDevice 충돌(070 panic) 방지를 위해 소리 대신 LED로 대체                    
            }
        });
        basic.pause(2000);
    }
    ...
    // RGB 조명 관련 변수 제거 (StartbitRGBLight 의존성 완전히 제거)
...
//     }
  ...
  /**
    ...
    */
    ...
    /*
    // ===== RGB 조명 관련 기능 전체 제거 (StartbitRGBLight의 shim=sendBufferAsm 컴파일 문제로 인해)
    //       AiHand 손가락 제어에는 불필요한 기능이라 통째로 비활성화함
    //
